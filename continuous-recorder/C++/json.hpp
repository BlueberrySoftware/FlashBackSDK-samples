...
inline std::string wstring_utf8(const std::wstring &s)
{
	return std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>, wchar_t>{}.to_bytes(s);
}

inline std::wstring utf8_wstring(const std::string &s)
{
	return std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>, wchar_t>{}.from_bytes(s);
}

inline bool IsEqualCaseInsensitive(std::string strLeft, std::string strRight)
{
	return strLeft.size() == strRight.size() && 
		std::equal(strLeft.begin(), strLeft.end(), strRight.begin(), [](auto a, auto b) { return std::tolower(a) == std::tolower(b); });
}

inline std::shared_ptr<std::vector<uint8_t>> HexToBin(std::string& sHexString)
{
	auto lmbddecfromhex = [](int const x)
	{
		return x < 58 ? x - 48 : (x < 'a' ? x - ('A' - 10) : x - ('a' - 10));
	};
	std::shared_ptr<std::vector<uint8_t>> pVecResult = std::shared_ptr<std::vector<uint8_t>>(new std::vector<uint8_t>(sHexString.length() / 2));
	uint8_t* pDstBuffer = pVecResult->data();
	const char* pSrcBuffer = sHexString.data();
	for (int32_t i = 0; i < (int32_t)pVecResult->size(); i++)
	{
		*pDstBuffer = (lmbddecfromhex(*pSrcBuffer) << 4) + lmbddecfromhex(*(pSrcBuffer + 1));
		pSrcBuffer += 2;
		pDstBuffer++;
	}
	return pVecResult;
...
inline std::shared_ptr<std::vector<uint8_t>> HexToBinRGBAExchangeRedAndBlue(std::string& sHexString)
{
	auto lmbddecfromhex = [](int const x)
	{
		return x < 58 ? x - 48 : (x < 'a' ? x - ('A' - 10) : x - ('a' - 10));
	...
	uint8_t* pDstBuffer = pVecResult->data();
	const char* pSrcBuffer = sHexString.data();
	for (int32_t i = 0; i < (int32_t)pVecResult->size() / 4; i++)//4 bytes per pixel
	{
		pDstBuffer[2] = (lmbddecfromhex(*pSrcBuffer) << 4) + lmbddecfromhex(*(pSrcBuffer + 1));
		pSrcBuffer += 2;
		pDstBuffer[1] = (lmbddecfromhex(*pSrcBuffer) << 4) + lmbddecfromhex(*(pSrcBuffer + 1));
		pSrcBuffer += 2;
		pDstBuffer[0] = (lmbddecfromhex(*pSrcBuffer) << 4) + lmbddecfromhex(*(pSrcBuffer + 1));
		pSrcBuffer += 2;
		pDstBuffer[3] = (lmbddecfromhex(*pSrcBuffer) << 4) + lmbddecfromhex(*(pSrcBuffer + 1));
		pSrcBuffer += 2;
		pDstBuffer += 4;
	}
	return pVecResult;
